...
class DockerTestnet {
  ...
  async start({ detached = true, extraParams = '' } = {}) {
    const nodeos = run(
      `docker run --rm --name eosio ${detached ? '-d' : ''} \
      -p 8888:8888 \
    ...
    const waitAndLoadContracts = async () => {
      await new Promise(resolve => setTimeout(resolve, 3000))
      await run(
        'docker exec -it eosio /opt/eosio/bin/cleos get info',
        this.printOutput && detached
      )
      await this.operator.loadSystemContracts()
    }
    if (detached) {
      await nodeos
      await waitAndLoadContracts()
    } else {
      waitAndLoadContracts().catch(error => {
        console.error(error)
        process.exit(1)
      })
    }
