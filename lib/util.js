...
async function giveCodeActivePermission(account, contract) {
  const auth = (await eos.getAccount(account)).permissions.find(
  ...
  await eos.updateauth(account, 'active', 'owner', auth, {
    authorization: account
  })
}
module.exports.giveCodeActivePermission = giveCodeActivePermission
